...
const scopedServices = await discoverServices({ env: args.env, venture: args.venture });
...
for (const service of services) {
  const outputDirectory = path.join("generated", target, service.metadata.env, service.metadata.venture, service.metadata.serviceName);
