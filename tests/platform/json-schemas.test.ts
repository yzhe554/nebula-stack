...
import { z } from "zod";
import { generateSchemaObject, platformJsonSchemas } from "../../src/platform/schema-json.js";
...
describe("service JSON schemas", () => {
  test("generates JSON schemas through the Zod schema instance", () => {
    ...
    expect(generateSchemaObject(schema, {
      id: "https://example.local/schemas/test.schema.json",
      title: "Test Schema",
  ...
  test("lambda schema validates the sample Lambda YAML", async () => {
    ...
    const yaml = parse(await readFile("services/dev/venture/core/internal/payment-api.lambda.yaml", "utf8"));
  ...
  test("dynamodb schema validates the sample DynamoDB YAML", async () => {
    ...
    const yaml = parse(await readFile("services/dev/venture/core/restricted/customer-records.dynamodb.yaml", "utf8"));
  ...
  test("network schema validates the sample network policy", async () => {
    ...
    const yaml = parse(await readFile("services/dev/venture/core/network.yaml", "utf8"));
