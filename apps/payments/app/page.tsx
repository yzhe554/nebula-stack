...
export default function PaymentsPage() {
  ...
  const paymentApiBaseUrl = useMemo(
    () => trimTrailingSlash(process.env.NEXT_PUBLIC_PAYMENT_API_BASE_URL ?? ""),
    [],
  );
  ...
  async function submitPayment(event: FormEvent<HTMLFormElement>) {
    ...
    if (!isConfigured) {
      setState({
        status: "error",
        message: "Set NEXT_PUBLIC_PAYMENT_API_BASE_URL before submitting payments.",
      });
      return;
    }
    ...
    <main className="shell">
      ...
      <section className="panel">
        ...
        <div className="result" aria-live="polite">
          ...
          {!isConfigured ? (
            <p className="warning">
              Configure <code>NEXT_PUBLIC_PAYMENT_API_BASE_URL</code>. For Floci this looks like{" "}
