...
describe("network zone policies", () => {
  test("loads zone policy for an environment venture and vpc", async () => {
    ...
    await writeFile(path.join(platformRoot, "services", "dev", "venture", "core", "network.yaml"), [
      "cidrs:",
      "  ipv4:",
      "    vpc: 10.20.0.0/16",
      "zones:",
    ...
    const policy = await loadNetworkPolicy({ env: "dev", venture: "venture", vpc: "core", servicesRoot: path.join(platformRoot, "services") });
    ...
    expect(policy.flows).toContainEqual({ from: "internal", to: "aws", services: ["dynamodb", "logs"] });
  ...
  test("requires explicit DynamoDB gateway endpoint config when flows use DynamoDB", async () => {
    ...
    await expect(loadNetworkPolicy({ env: "dev", venture: "venture", vpc: "core", servicesRoot }))
      .rejects
      .toThrow("awsEndpoints.dynamodb is required");
  });
  ...
  test("requires DynamoDB services to have a default gateway endpoint", async () => {
    ...
    await writeFile(path.join(servicesRoot, "dev", "venture", "core", "managed", "customer-records.dynamodb.yaml"), [
    ...
    await expect(discoverServices({ env: "dev", venture: "venture", servicesRoot }))
      .rejects
      .toThrow("DynamoDB service customer-records requires awsEndpoints.dynamodb default gateway endpoint");
  });
  ...
  test("service discovery rejects a security zone that is not defined for the vpc", async () => {
    ...
    await writeFile(path.join(servicesRoot, "dev", "venture", "core", "unknown", "payment-api.lambda.yaml"), [
      "runtime: nodejs22.x",
    ...
    await expect(discoverServices({ env: "dev", venture: "venture", servicesRoot }))
      .rejects
      .toThrow("Security zone unknown is not defined");
  });
});
